...
function resultToComponent(expressions, results) {
    console.log('resultToComponent', expressions, results);
  const elmMakeErrors = results.filter(r => r.status === 'rejected')
  elmMakeErrors.forEach((r, i) => {
      console.error(r.e);
      throw new e;
  })
  const errorIndices = results.map((r, i) => i).filter(i => results[i].status === 'rejected')
  const resolvedExpressions = expressions
                  .map((a, i) => ({ expression: a,
                                    pairs: (results[i].status === 'resolved')
                                         ? results[i].v.types.map((type, j) => {
                                             return {
                                                 type: type,
                                                 value: results[i].v.values[j]
                                             }
                                         })
                                         : null }))
  ...
  const elmComponents = resolvedExpressions.map((result, index) => {
    ...
    //if (source && source.embed) {
      return (
        ...
        result.pairs.map((pair) => (
            <div>
                <span>{ Repl.stringify(pair.type) }</span>
...
function compileElmFiles(expressions) {
  const allPromises = expressions.map((expression) => {
    ...
    console.log('command', expression.commands)
    const imports = expression.commands.filter((command) => command.type == 'import')
                                       .map((command) => command.value)
    const code = expression.commands.filter((command) => command.type == 'expression')
                                    .map((command) => command.value)
    ...
    return new Repl({
        ...
        elmVer: '0.18.0',
        keepTempFile: true
    }).getTypesAndValues(imports, code)
  })
...
export function compile(code: string = '', playgroundCode: string = '', openFilePath: string) {
    //console.log('compile function start')
  ...
  console.log('compile function middle')
  ...
  return updateFileSources(openFileFolderPath, lastOpenFilePath, packageJsonTemplateFileContents)
    ...
    .then(({allPromises, expressions}) => { // eslint-disable-line
        console.log(allPromises, expressions);
